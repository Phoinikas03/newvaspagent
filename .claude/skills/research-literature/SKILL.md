...
description: "为计算材料学任务结构化检索与整理文献：实验晶格常数、带隙、DFT/HSE/VASP 参数、DFT+U、EOS 方法等。在 workflow-relax、workflow-electronic-structure、workflow-eos-lattice-constant、workflow-convergence 等技能要求「先查文献」或本地 references 未覆盖时触发；统一使用本 skill 规定的检索顺序与 arXiv 查询规范，输出带引用的摘要并可写入用户指定的工作区 Markdown。"
version: "1.0.0"
---
...
- 其它 skill 写明「仅当本地 `references/*.md` 未覆盖时调用 `Skill: research-literature`」时——**先读本 skill**，再按下列流程检索，**不要**跳过本 skill 直接乱用 `arxiv_search`（避免查询词不当、结果不可复现）。
...
- **`workflow-convergence`**：本 skill **不**替代收敛测试；仅提供文献中的 ENCUT/K 点经验作**初值参考**，真正收敛仍以该 skill 与 `Convergence_Report.md` 为准。
- **`workflow-eos-lattice-constant`**：实验晶格常数对比值优先通过本 skill 或 Materials Project 实验数据交叉核对。
- **`workflow-relax` / `workflow-electronic-structure`**：当 `references/incar_params.md`、`hse_params.md` 等未覆盖时，用本 skill 补参数与实验带隙引用。
...
- **不替代计算**：文献参数是参考；与用户确认的 **VASP 实际输入**仍以各计算 skill 与 `setup_vasp_inputs`、`run-vasp` 为准。

---

## 与「ITERATIVE EXECUTION RULE」的关系

本 skill **不涉及**在单脚本中循环提交 VASP。若检索目的是为**下一步**收敛或 EOS 提供参数，真正跑 VASP 时须遵守项目 system prompt 与 **`run-vasp`** skill，逐点提交、逐步核查。
