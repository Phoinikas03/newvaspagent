...
**收敛测试默认执行**：ENCUT/KSPACING 收敛是本工作流的**标准步骤**，默认进行，不需要事先征得同意。仅在用户**明确要求跳过**、或工作区已有**可信且适用**的 `Convergence_Report.md` 时才可省略。跳过时必须在报告中显著注明。见下文 §0。
...
ENCUT/KSPACING 收敛测试是本工作流的默认步骤。**不要**为了询问是否执行而停下来——
下游结果的可信度依赖它，而在无人值守或「参数由你决定」的任务里，停下来询问的结果
往往是整个步骤被静默跳过。

1. 用**纯文本**向用户**说明**（不是征询）：即将进行 ENCUT 与 KSPACING 收敛测试，
   包含多组静态单点计算（`NSW=0`），并给出大致的计算点数与机时量级。说明后**直接进入 §1**。
2. **仅**在下列情形可以跳过收敛扫描：
   - 用户在本次对话中**明确要求**跳过、或明确指定了要直接使用的 ENCUT/KSPACING；
   - 工作区已有**可信且适用于当前体系**的 `Convergence_Report.md`（同一元素、同一赝势、
     同一结构类型），此时复述其结论并复用。
3. 跳过时**必须**：
   - 在对话中说明跳过的原因（属于上述哪一种）；
   - 在最终报告的**结论附近**（不是脚注）写明「**未做系统收敛测试**」及所用参数的来源；
   - 在工作区写入 `CONVERGENCE_SKIPPED.md`，记录原因、采用的 ENCUT/KSPACING 及其依据。
4. 「任务描述把参数决定权交给你」**不构成**跳过的理由。参数由你决定，正意味着
   应当由收敛测试来确定它们，而不是凭经验直接取值。
...
- **默认执行**：收敛测试默认进行，无需事先取得同意；跳过必须满足 §0 第 2 条并留下 `CONVERGENCE_SKIPPED.md`。「参数由你决定」不是跳过的理由。
