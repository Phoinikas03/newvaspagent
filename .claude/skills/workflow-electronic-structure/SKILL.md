---
name: "workflow-electronic-structure"
description: "执行 VASP 电子结构工作流：静态 SCF、带结构、DOS，以及高精度 HSE 带隙。它是在原 bandgap skill 基础上的升级版，统一收纳 PBE→HSE 带隙、band structure、DOS 等电子结构任务。真正运行前必须经过 run-vasp；涉及 GPU、KPAR、NCORE、NPAR 时必须先经过 incar-performance。"
---
...
- `run-vasp`：正式运行入口，必须使用
- `incar-performance`：只要用户提到 GPU、`KPAR`、`NCORE`、`NPAR` 或希望按硬件优化时，必须先用
- `workflow-convergence`：正式电子结构对比前建议先做 `ENCUT/KSPACING` 收敛
- `workflow-relax`：若结构还未优化，优先先松弛
- `vasp-error-recovery`：出错、卡住、超时时的诊断与恢复
...
步骤：
1. 确认结构来源
2. 视需要先做 `workflow-convergence`
3. 读取并写入 `templates/INCAR_pbe_scf`
4. 生成输入文件
5. 通过 `run-vasp` 的 `vasp_runner.py` 提交
6. 用 `run-vasp/scripts/check_convergence.py` 检查
7. 若失败、未收敛或疑似卡住，优先转给 `vasp-error-recovery`
...
- 若非自洽阶段异常退出或后处理文件不完整，优先用 `vasp-error-recovery` 判断是否是输入问题、收敛问题，还是仅后处理不完整
...
无论是静态 SCF、band structure、DOS，还是 HSE，只要出现以下任一情况，都应先转给 `vasp-error-recovery`：

- `vasp_runner.py` 非零退出
- `run-vasp/scripts/check_convergence.py` 返回 `failed` / `unconverged` / `incomplete_postprocess`
...
3. 若仍需统一判断“是否继续等待、是否要 terminate、如何修改后重跑”，再调用 `vasp-error-recovery`

推荐调用：

```bash
cd "<repo_root>" && python .claude/skills/vasp-error-recovery/scripts/analyze_error.py --work-dir "<当前任务目录>"
```

根据 `vasp-error-recovery` 的输出来决定：

- 是否继续等待
- 是否修改 `NELM` / `ALGO` / `ISMEAR` / `SIGMA` / HSE 参数
- 是否建议先 terminate 旧 run 再重新提交

若 `vasp-error-recovery` 建议先停旧 run，必须先向用户说明证据、拟修改项和新的 runner 命令。只有在用户明确同意后，才允许：

```bash
cd "<repo_root>" && python .claude/skills/run-vasp/scripts/terminate.py --work-dir "<当前任务目录>" --reason "<停止原因>"
```
...
4. **出错先诊断再决定是否停**
   若 HSE 因 GPU/MPI/内存问题异常，优先用 `vasp-error-recovery` 看是否应先 terminate，再修改参数并重新提交。
...
```bash
cd "<repo_root>" && python .claude/skills/workflow-electronic-structure/scripts/gap.py "<vasprun.xml>"
```
...
- 不得在未获用户确认时直接启动 HSE
- 不得在用户要求按硬件优化时跳过 `incar-performance`
- 不得跳过 `vasp-error-recovery`，直接对失败或疑似卡住的旧 run 做模糊终止或盲目重跑
