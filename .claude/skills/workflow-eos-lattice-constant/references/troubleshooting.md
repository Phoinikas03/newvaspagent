...
- 若最低点已被覆盖但底部点数太少或 `fit_eos.py` 输出 `refinement_recommended: true`，不要只扩大范围；应围绕 `linear_scale_eq` 用 `0.003-0.005` 的线性 scale 步长二次加密，再用首轮 + 新增点重新拟合。
- 各点 **ENCUT / KSPACING（K 点网格）/ POTCAR** 是否完全一致（否则能量不可比）。
...
- `templates/INCAR_static` 位于 `workflow-eos-lattice-constant/templates/`，复制到各 `scale_*` 目录后再改 `ENCUT`。
