#!/usr/bin/env python
"""VASP Agent arm of the band-gap pilot.
...
# The functional is part of the task specification, not something either arm is
# asked to discover: atomate2's band-structure default is PBE, and comparing a
# PBE gap against an HSE one would measure the functional, not the workflow.
# Everything else -- cutoff, k-sampling, smearing, pseudopotential variant, how
# the gap is extracted -- is left to the agent, as in the Sol27LC arm.
TASK_PROMPT = """\
请计算工作目录中 POSCAR 所描述材料的带隙。
...
要求：
- 使用 HSE06 杂化泛函精度。
- 按 bandgap 工作流**完整**执行，不要省略其中的步骤。
- 最终请明确报告带隙数值（单位 eV）及其类型（直接/间接）。

我不会为你指定任何计算参数（ENCUT、K 点、smearing、赝势变体、AEXX/HFSCREEN、ALGO 等）。
...
def build_system_prompt(workspace: str) -> str:
    return "\n".join(
        [
            ...
            "",
            "任务类型: HSE06 带隙计算。除泛函已指定为 HSE06 外，所有计算参数由你自行决定。",
        ]
    )


def looks_finished(workspace: Path) -> bool:
    """Done when a converged hybrid run with readable eigenvalues exists.

    Judged from vasprun.xml, never from what the agent says and never from a
    summary file it may or may not choose to write.
    """
