#!/usr/bin/env python
"""VASP Agent arm of the SR (structure relaxation) repeated-run experiment.

A copy of rev_sol27lc/scripts/run_agent_lc.py with the task swapped: same
model routing, system-prompt rules, question handling, idle back-off and
upstream-failure abort. What differs is listed in protocol/CHANGELOG.md
(task prompt, completion test, budget, answer rules).

Task specification is "T2", as for Sol27LC: the agent gets a POSCAR and the
goal and nothing else -- no ENCUT, k-spacing, smearing, DFT+U, magnetic moments
or ISIF. It may ask questions; every reply comes from the pre-registered
deterministic script in protocol/answer_script.py, never from a model.
"""
...
REPO = Path(__file__).resolve().parents[2]  # <repo>/rev_relax/scripts/<this>
EXP = REPO / "rev_relax"
sys.path.insert(0, str(REPO))
sys.path.insert(0, str(EXP / "protocol"))

# 15-95 atom vacancy supercells on one GPU: the archived rx_* sessions needed
# up to 19 h on eight GPUs, so the Sol27LC 6 h budget would censor the slow
# systems in every rep. Fixed at 24 h before the first run (operator, 09-24).
DEFAULT_BUDGET_SEC = 24 * 3600
# The flash model yields in short turns; rounds are not the budget, wall time is.
MAX_ROUNDS = 300

# Same shape as the Sol27LC prompt. "晶格与原子位置全松弛" states the goal the
# expert reference was computed for (ISIF=3); it names no parameter. The
# archived SR opening was "我要计算<path>的结构优化".
TASK_PROMPT = """\
请对工作目录中 POSCAR 所描述的结构进行结构优化（晶格与原子位置全松弛）。
...
- 使用 PBE 泛函。
- 按 relax 工作流执行。
- 结束时请明确报告：是否收敛、最终总能量、最大残余力、最终结构文件（CONTCAR）的路径。

我不会为你指定任何计算参数（ENCUT、K 点、smearing、赝势变体、DFT+U、磁矩、ISIF 等）。
"""
...
def looks_finished(workspace: Path) -> bool:
    ...
    (see check_flow.py: a converged, normally terminated relaxation with a
    CONTCAR and no VASP still running in the workspace)."""
    ...
    try:
        async with ClaudeSDKClient(options=options) as client:
            ...
            for rnd in range(MAX_ROUNDS):
                ...
                asking = not had_tool_use and awaits_reply(last_text)
                # Unlike an EOS fit, a converged relaxation is not obviously the
                # last step: the agent may want a second pass from CONTCAR. So a
                # round that ends in a question is answered first; the run stops
                # on the first round that ends without one. A closing "any
                # follow-up calculation?" is answered "no, finish" and the next
                # round ends it.
                if looks_finished(outdir) and not asking:
                ...
                if asking:
                    ...
                    if repeats >= STUCK_LIMIT and looks_finished(outdir):
                        # Finished on disk and only chatting now (typically the
                        # closing follow-up question, asked again and again).
                        info["status"] = "completed"
                        info["completed_note"] = f"stopped on repeated [{category}] question after completion"
                        break
        ...
        # run_batch.sh frees this GPU as soon as the driver exits; a VASP run
        # the agent left behind (budget hit mid-relaxation, exception) would
        # then share the card with the next system and spoil both wall clocks.
        sys.path.insert(0, str(Path(__file__).resolve().parent))
        from check_flow import vasp_running_in

        leftover = vasp_running_in(outdir)
        for pid in leftover:
            try:
                os.kill(pid, 15)
            except OSError:
                pass
        if leftover:
            info["killed_vasp_pids"] = leftover
            transcript.write(f"\n=== driver exit: terminated leftover VASP {leftover} ===\n")
        transcript.close()
        event_log.close()
...
def main() -> None:
    ...
    outdir.mkdir(parents=True, exist_ok=True)
    if (outdir / "run_meta.json").exists() or (outdir / "driver.log").exists():
        # A restarted system must start from an empty workspace (HANDOVER §4).
        sys.exit(f"{outdir} holds an earlier attempt; move it to .{args.system}.crashed first")
