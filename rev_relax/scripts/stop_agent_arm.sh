...
signal_matching '*run_batch.sh rep*' TERM batch
sleep 2
echo "stopping agent drivers"
signal_matching '*python*run_agent_relax.py*' TERM driver
sleep 6
...
for p in $(ls /proc | grep -E '^[0-9]+$'); do
  ...
  [ "$p" = "$PARENT" ] && continue
  cmd=$(tr '\0' ' ' < "/proc/$p/cmdline" 2>/dev/null) || continue
  case "$cmd" in *python*run_agent_relax.py*|*run_batch.sh\ rep*) echo "  STILL: $p"; left=$((left+1)) ;; esac
done
