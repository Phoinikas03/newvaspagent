...
LOG="${BG_WATCH_LOG:-$RUNS/claude_bandgap_watch.log}"
...
CLAUDE_PRINT_TIMEOUT="${BG_CLAUDE_PRINT_TIMEOUT:-300}"
...
run_watch_iteration() {
  ...
  msg_file="$(mktemp /tmp/claude_bandgap_watch_msg.XXXXXX)" || {
    log "[warn] mktemp 失败"
    return 0
  }
  prompt="$(build_prompt)"
...
{
  echo "==== claude bandgap watch 开始 $(date -Is) ===="
